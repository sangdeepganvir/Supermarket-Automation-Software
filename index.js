...
//manager
app.post("/removeitem", bodyParser, async (req, res) => {
  // res.send({ resp: req.body });
  try {
    await Item.findOneAndRemove(req.body.id);
    res.send({ resp: "ok" });
  } catch (e) {
    res.send({ resp: "no" });
  }
...
app.get("/stat", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    console.log({ allsales, allDetails, allsalesforpie, filter });
    // const main=true;
...
app.post("/bill", isLoggedIn, async (req, res) => {
  ...
  for (let i = 0; i < bill.code.length; i++) {
    ...
    bill_items.push({
      ...
      customer_name: bill.customer_name[i],
      contact: bill.contact[i],
    });
...
app.post("/saveemail", (req, res) => {
  var { customer_name, contact, email } = req.body;
  const newItemData = {
  ...
  const newItem = new Subs(newItemData);
